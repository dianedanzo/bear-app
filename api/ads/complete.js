...
export default async function handler(req, res) {
  try {
    ...
    const { reward, ref, block_id } = req.body || {};
    const refId = String(ref || block_id || '');
    if (!reward || !refId) return res.status(400).json({ ok: false, error: 'missing_params' });
    const sb = svc();
    const { error } = await sb.rpc('complete_ad_task', {
